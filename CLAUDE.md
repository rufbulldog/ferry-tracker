...
## Auto-generated spec (`docs/spec/`)

Machine-readable **L1 (per-file)** + **L2 (per-folder)** specs for `app/`, `src/`, and the CDK `infra/` — a fast way to see a folder's public surface, exports, and imports without reading every file. Generated by the shared `spec` CLI (private [`rufbulldog/dev-utils`](https://github.com/rufbulldog/dev-utils), `packages/spec`; structure declared in [`spec.config.json`](spec.config.json)). No SAM here, so there's no system-level (L3) spec — L1/L2 only. Keep them current with `spec extract` after code changes; `/ship` step 2 does this automatically. There's no CI spec-gate (iOS-only, no GitHub Actions), so the `/ship` reconcile is the enforcement.
...
- **Lint is split.** The app is linted from the repo root (`npm run lint`, scoped to `app src`) via the shared `@rufbulldog/eslint-config`. The CDK backend has its own standalone `typescript-eslint` setup — run `cd infra && npm run lint`. The two configs are independent; the shared Expo config does not cover `infra/`.
- **Versioning.** The Settings screen shows the **native** version + build number — `Application.nativeApplicationVersion (nativeBuildVersion)` from `expo-application` (`app/(tabs)/settings.tsx`), so the label matches App Store Connect / TestFlight exactly and never drifts when an OTA lands on a build. `app.json` `expo.version` is still the source the build is stamped from, and `/ship` bumps it every deploy (patch by default; step 7 of the playbook). `runtimeVersion` is normally **pinned** and decoupled from `version` so OTA ships can bump the version freely without orphaning updates — it only moves on a native rebuild, to the new version. Never bump `runtimeVersion` on an OTA-only ship.
