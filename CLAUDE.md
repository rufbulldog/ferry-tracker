...
- **Versioning.** The Settings screen shows `app.json` `expo.version` (via `Constants.expoConfig.version` — single source of truth, not hardcoded). `/ship` bumps `expo.version` every deploy (patch by default; step 7 of the playbook). `runtimeVersion` is **pinned** to a fixed string in `app.json` (decoupled from `version`) so OTA ships can bump the version freely without orphaning updates — it only moves on a native rebuild, to the new version. Never bump `runtimeVersion` on an OTA-only ship.
